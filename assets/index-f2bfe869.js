...
`;document.getElementById("loader").innerHTML=h;var y=document.getElementById("loader");window.addEventListener("load",function(){y.style.display="none"});const u=`
...
`;document.getElementById("header").innerHTML=u;const n=document.querySelector("header");window.addEventListener("scroll",()=>{window.scrollY>0?n.classList.add("scroll"):n.classList.remove("scroll")});let p;fetch("/assets/movies-db.json").then(i=>i.json()).then(i=>{p=i,l("movie-1"),l("movie-2"),l("movie-3"),l("movie-4"),l("movie-5"),l("movie-6")});function l(i){const e=p[i],s=document.getElementById(`hero-slider-${i.split("-")[1]}`);var r;i==="movie-1"?r="eger":r="lazy",s.innerHTML=`

  <div class="carousel-item active relative float-left w-full">
  <img class="pl-20 bg-gray-800" src="${e.imagemain}" class=" h-[750px] w-full" alt="${e.title}" loading="${r}" />
...
background: linear-gradient(90deg, rgba(0,0,0,1) 0%, rgba(53,50,50,0.8380602240896359) 71%,rgba(53,50,50,0.6380602240896359) 81%,rgba(53,50,50,0.3380602240896359) 95%, rgba(0,0,0,0) 100%);">
      <img width="400px" src="${e.imagetext}" alt="${e.title}" loading="${r}"/>
      <div class="space-y-4">
...
`;document.getElementById("sliderDisplay").innerHTML=m;const g=`
<div
...
`;document.getElementById("footer").innerHTML=g;let c;fetch("/assets/movies-db.json").then(i=>i.json()).then(i=>{c=i;for(let e=7;e<=38;e++){const s=`movie-${e}`,r=f(s);console.log(r)}});function f(i){const e=c[i],s=document.getElementById(`card-${i.split("-")[1]}`);let r;i==="movie-14"?r="9":r="0",s.innerHTML=`

    <div class="mt-5 pl-9 pr-${r}"> 
...
<div class=" card container relative w-[300px]  items-center duration-150 ease-in hover:scale-125 overflow-hidden rounded-xl cursor-pointer">
  <img width="300" class="block rounded-xl" src="${e.cardimage}" alt="${e.title}" loading="lazy" />
  <div class="overlay absolute bottom-0 flex w-[300px] justify-between space-y-3 bg-clip-padding px-4 py-3  text-white opacity-0 ease-in-out">
    <div class=" description space-y-1">
      ...
      <p class="text-[8px] text-white">
        <span>${e.releseyear} &nbsp;|&nbsp;&nbsp;</span>
        <span class="p-[1px] ring-1 ring-white">&nbsp;${e.maturityrating} </span>
        <span>&nbsp;&nbsp;&nbsp;|&nbsp; ${e.runtime}&nbsp;|&nbsp;</span>
        <span>&nbsp;${e.movietype}</span>
      </p>
    </div>
...
<h1 class=" bg-black text-white   text-[14px] font-medium mt-2 pl-5 ">${e.title}
</h1>
</div>
...
`}const b=`
  ...
  `;document.getElementById("carasol-content-1").innerHTML=b;document.getElementById("carasol-content-2").innerHTML=x;document.getElementById("carasol-content-3").innerHTML=z;document.getElementById("carasol-content-4").innerHTML=k;function E(i){var e=document.createElement("iframe");e.setAttribute("src","https://www.youtube.com/embed/"+i.dataset.id+"?autoplay=1&rel=0"),e.setAttribute("frameborder","0"),e.setAttribute("allowfullscreen","1"),e.setAttribute("allow","accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture"),i.parentNode.replaceChild(e,i)}function C(){for(var i=document.getElementsByClassName("youtube-player"),e=0;e<i.length;e++){var s=i[e].dataset.id,r=document.createElement("div");r.setAttribute("data-id",s);var t=document.createElement("img");t.src="//i.ytimg.com/vi/ID/hqdefault.jpg".replace("ID",s),r.appendChild(t);var a=document.createElement("div");a.setAttribute("class","play"),r.appendChild(a),r.onclick=function(){E(this)},i[e].appendChild(r)}}document.addEventListener("DOMContentLoaded",C);let v;fetch("/assets/video-id.json").then(i=>i.json()).then(i=>{v=i;for(let e=1;e<=38;e++){const s=`video-${e}`,r=L(s);console.log(r)}});function L(i){const e=v[i],s=document.getElementById(`popup-${i.split("-")[1]}`);s.innerHTML=`
...
`}const $=38;let w="";for(let i=1;i<=$;i++)w+=`<div id="popup-${i}" class="popup"></div>`;document.getElementById("video-dispaly").innerHTML=w;class d extends HTMLElement{connectedCallback(){this.videoId=this.getAttribute("videoid");let e=this.querySelector(".lty-playbtn");if(this.playLabel=e&&e.textContent.trim()||this.getAttribute("playlabel")||"Play",this.style.backgroundImage||(this.style.backgroundImage=`url("https://i.ytimg.com/vi/${this.videoId}/hqdefault.jpg")`),e||(e=document.createElement("button"),e.type="button",e.classList.add("lty-playbtn"),this.append(e)),!e.textContent){const s=document.createElement("span");s.className="lyt-visually-hidden",s.textContent=this.playLabel,e.append(s)}e.removeAttribute("href"),this.addEventListener("pointerover",d.warmConnections,{once:!0}),this.addEventListener("click",this.addIframe),this.needsYTApiForAutoplay=navigator.vendor.includes("Apple")||navigator.userAgent.includes("Mobi")}static addPrefetch(e,s,r){const t=document.createElement("link");t.rel=e,t.href=s,r&&(t.as=r),document.head.append(t)}static warmConnections(){d.preconnected||(d.addPrefetch("preconnect","https://www.youtube-nocookie.com"),d.addPrefetch("preconnect","https://www.google.com"),d.addPrefetch("preconnect","https://googleads.g.doubleclick.net"),d.addPrefetch("preconnect","https://static.doubleclick.net"),d.preconnected=!0)}fetchYTPlayerApi(){window.YT||window.YT&&window.YT.Player||(this.ytApiPromise=new Promise((e,s)=>{var r=document.createElement("script");r.src="https://www.youtube.com/iframe_api",r.async=!0,r.onload=t=>{YT.ready(e)},r.onerror=s,this.append(r)}))}async addYTPlayerIframe(e){this.fetchYTPlayerApi(),await this.ytApiPromise;const s=document.createElement("div");this.append(s);const r=Object.fromEntries(e.entries());new YT.Player(s,{width:"100%",videoId:this.videoId,playerVars:r,events:{onReady:t=>{t.target.playVideo()}}})}async addIframe(){if(this.classList.contains("lyt-activated"))return;this.classList.add("lyt-activated");const e=new URLSearchParams(this.getAttribute("params")||[]);if(e.append("autoplay","1"),e.append("playsinline","1"),this.needsYTApiForAutoplay)return this.addYTPlayerIframe(e);const s=document.createElement("iframe");s.width=560,s.height=315,s.title=this.playLabel,s.allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture",s.allowFullscreen=!0,s.src=`https://www.youtube-nocookie.com/embed/${encodeURIComponent(this.videoId)}?${e.toString()}`,this.append(s),s.focus()}}customElements.define("lite-youtube",d);new Swiper(".mySwiper",{slidesPerView:1,spaceBetween:0,autoplay:{delay:5e3,disableOnInteraction:!1},loop:!0,navigation:{nextEl:".swiper-button-next",prevEl:".swiper-button-prev"}});new Swiper(".mySwiper-1",{slidesPerView:5,spaceBetween:70,loop:!1,navigation:{nextEl:".swiper-button-next",prevEl:".swiper-button-prev"}});new Swiper(".mySwiper-2",{slidesPerView:5,spaceBetween:70,loop:!1,navigation:{nextEl:".swiper-button-next",prevEl:".swiper-button-prev"}});
